...
import { decodeState, nonceStorageKey } from '@/lib/social-oauth';
import type { SocialOAuthState, SocialProvider } from '@/lib/social-oauth';

const DEFAULT_REDIRECT = '/my-page';

/** Where to send the user back to on error, based on the originating page. */
function originPath(state: SocialOAuthState | null): string {
...
function SocialCallback({ provider }: SocialOAuthCallbackProps) {
  ...
  useEffect(() => {
    ...
    const storedNonce = window.sessionStorage.getItem(nonceStorageKey(provider));
    window.sessionStorage.removeItem(nonceStorageKey(provider));

    if (oauthError) {
      // User cancelled at the provider consent screen, or the provider
      // returned an OAuth error (e.g. access_denied).
      fail(
        oauthError === 'access_denied'
          ? 'social_callback.error_cancelled'
          : 'social_callback.error_failed',
      );
      return;
    }

    if (!code || !state || !storedNonce || storedNonce !== state.nonce) {
      fail('social_callback.error_failed');
      return;
    }
    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-gray-light px-4">
      {error ? (
        <div className="flex w-full max-w-[420px] flex-col items-center gap-4 rounded-xl bg-white p-8 text-center shadow-lg">
          ...
          >
            {t('social_callback.back_to_sign_in')}
          </button>
        </div>
      ) : (
        <div
          className="flex flex-col items-center gap-4 text-center"
          data-testid="social-callback-loading"
        >
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-gray-200 border-t-green-dark" />
          <p className="text-gray-text">{t('social_callback.completing')}</p>
        </div>
      )}
    </main>
  );
...
export default function SocialOAuthCallback({ provider }: SocialOAuthCallbackProps) {
  return (
    <Suspense fallback={null}>
      <SocialCallback provider={provider} />
    </Suspense>
  );
}
