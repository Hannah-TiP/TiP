...
// `notes: 'review_deleted'`. Markers drive the wallet's kind qualifier and
// are never rendered as a note. Kept separate from `types/stay-credit.ts`
// (which owns the source/kind label maps) so the marker vocabulary has one
// home.
...
const HIDDEN_NOTE_MARKERS: ReadonlySet<string> = new Set([REVIEW_DELETED_NOTE_MARKER]);
...
/**
 * Kind qualifier implied by a note marker ("Review deleted / 후기 삭제"), or
 * null when the generic `POINT_TRANSACTION_KIND_LABELS` map applies.
 */
export function noteMarkerKindQualifier(row: PointTransaction, en: boolean): string | null {
  if (isReviewDeletedClawback(row)) return REVIEW_DELETED_KIND_LABEL[en ? 'en' : 'kr'];
  return null;
}
